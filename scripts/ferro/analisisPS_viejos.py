...
def load(archivo):
    ...
    print(data[:4000,:])
    try:
        data = np.hsplit(data[:4000,:],4)
    except ValueError:
        data = np.hsplit(data[:4000,:],3)
    return data
...
def main(archivos, color, medicion):
    ...
    for archivo in archivos:
        ...
        # plt.plot(P, S, label=f"T = {T+273:.2f}K", markersize=1)
        if np.mean(T)>-40:
            plt.plot(P, S, 'o',label=f"T = {T:.2f}C, U = {np.mean(U):.2f}", markersize=1)
        else: pass
        # plt.plot(U, m, '.', c='b')
...
for i in mediciones:
    ...
    dir = "/home/marco/Documents/fac/labo4/ferro/mediciones/viejos"
    archivos = sorted(os.listdir(dir))
    # print(f"mediciones de medicion{i+1}")
    print(f"mediciones de viejos")
    print(len(archivos))
