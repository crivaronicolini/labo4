...
def plotear2(X,V):
    try:
        ...
        plt.fill_between(x,varmenos,varmas,color='k', alpha=0.5)
        plt.fill_betweenx(varvals, x-xerr,x+xerr,color='k',alpha=0.5)
    except TypeError:
        print('no andan lo errores')
        plt.plot(S,var,'.')
...
def fiteo(x,y):
    ...
    # plt.plot(h,polinomio(h),'--w',label=fr'$\alpha =({z[0]:.7f} \pm {zerr:.7f}) \ mV \ K^{-1}$')
    #en volts da (0.053 +- 0,001)V
    plt.plot(h,polinomio(h),'--w',label=r'$\alpha =(5.9 \pm 0.1) \  mV \ K^{-1}$')
